...
export function HistoryPage() {
  ...
  if (!history) return <div className="p-6">Loading dossier...</div>

  return (
    <div className="space-y-12">
      <section className="space-y-4">
        <h1 className="font-display text-4xl text-primary font-semibold">Resolution Dossier</h1>
        <p className="text-muted-foreground text-lg max-w-2xl text-balance">
          Historical record of past races and their real-world outcomes.
        </p>
      </section>
        ...
        {history.runs.map((run: RunRecord) => (
          <div key={run.id} className="lab-card p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 hover:bg-black/5 transition-colors cursor-pointer">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 text-xs text-muted-foreground font-mono">
                  ...
                  <span>&bull;</span>
                  <span>{run.plans_count} Plans Evaluated</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-8 w-full md:w-auto border-t border-border md:border-none pt-4 md:pt-0">
              ...
              <div className="flex-1 md:flex-none">
                <div className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1">Empirical Match</div>
                <div className="flex items-center gap-1.5 text-sm font-medium">
                  {run.hit === true ? (
                    <span className="baseline-tag bg-green-100 text-green-800">Accurate</span>
                  ) : run.hit === false ? (
                    <span className="baseline-tag bg-red-100 text-red-800">Diverged</span>
                  ) : (
