...
export function SectionCard({
  ...
  return (
    <section
      className={`rounded-panel border border-line/80 bg-paper/95 shadow-card backdrop-blur ${className}`}
    >
      <div className="flex items-start justify-between gap-4 border-b border-line/70 px-5 py-4 sm:px-6">
        <div className="space-y-2">
          ...
          <div>
            ...
            {description ? (
              <p className="mt-1 max-w-2xl text-sm leading-6 text-ink/80">{description}</p>
            ) : null}
          </div>
        </div>
